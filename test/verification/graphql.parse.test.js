...
/**
 * Generate GraphQL SDL from a model and verify it parses with graphql-js.
 * @param {ModelManager} modelManager populated model manager
 * @param {object} [visitorOptions] options passed to GraphQLVisitor
 */
async function verifyGraphQLParses(modelManager, visitorOptions = {}) {
    ...
    try {
        ...
        const sdlPath = path.join(outputDir, 'model.gql');
...
describe('verification', function () {
    ...
    CASES.forEach(function (testCase) {
        ...
        const title = skipReason
            ? `generated GraphQL SDL from ${testCase.name} parses with graphql-js (pending: ${skipReason})`
            : `generated GraphQL SDL from ${testCase.name} parses with graphql-js`;
        const run = skipReason ? it.skip : it;

        run(title, async function () {
            const modelManager = createModelManager(testCase);
            await verifyGraphQLParses(modelManager, testCase.visitorOptions || {});
        });
    });
});
